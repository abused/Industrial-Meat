...
public class ManualAdditions {
    ...
    public static void addToManual() {
        addItemEntry(Item.getItemFromBlock(CommonProxy.MeatPacker), new PageText("The Meat Packer is used to create Raw Meat Ingots and nuggets. It takes in Liquid Meat and produces 1 ingot every 200mb and 1 nugget if there is less than 200 & < 50 mb in. Just give it power and get started. If no inventory is adjacent it will automatically drop the meat on top of it!"));
    ...
    public static void addItemEntry(Item item, IPage... pages) {
        BookCategory.ANIMAL_HUSBANDRY.getEntries().put(item.getRegistryName(), new CategoryEntry(item.getItemStackDisplayName(new ItemStack(item)), new ItemStack(item), Arrays.asList(pages)));
    }
}
